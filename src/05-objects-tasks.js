...
class CssBuilder {
  constructor() {
    ...
    this.doubleElementError = 'Element, id and pseudo-element should not occur more then one time inside the selector';
    this.orderElementsError = 'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element';
  }

  checkValue(string) {
    if (Object.prototype.hasOwnProperty.call(this.selectors, string)) {
      throw new Error(this.doubleElementError);
    }
  }
  ...
  checkPosition() {
    ...
    if (keys.some((selector, index) => index > currentPosition.indexOf(selector))) {
      throw new Error(this.orderElementsError);
    }
  }
